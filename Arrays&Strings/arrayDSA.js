 ...
 * Static arrays -> array that allocates a fixed amount of memory to be used for storing the array’s values
 * Dynaimc arrays -> allocates double the amount of memory needed to store the array’s values
*/
...
/** COMMON TERMS SEEN WHEN DOING PROBLEMS INVOLVING ARRAYS
    1. Subarray - A range of contiguous values within an array eg [2, 3, 6, 1, 5, 4] => [3, 6, 1] is a subarray
    2. Subsequence - A sequence that can be derived from the given sequence by deleting 
...
/** CALCULATING THE BASE ADDRESS OF AN ITEM STORED AT A GIVEN INDEX
 * calculated if we know the base address (where the array starts in memory) and the size of each data item
 * address[index] = base address + (size of datatype * index)
*/
